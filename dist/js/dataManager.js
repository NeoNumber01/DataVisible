...
class DataManager {
    constructor() {
        ...
        // Storage optimization: debounce timer and expiry settings
        this._saveTimer = null;
        this._saveDelay = 500; // 500ms debounce delay
        this._dataExpiry = 7 * 24 * 60 * 60 * 1000; // 7 days in milliseconds
    ...
    setData(data) {
        ...
        // Auto-save to localStorage with debounce (optimization)
    ...
    /**
     * Save current data to localStorage (immediate)
     */
    saveToStorage() {
        if (!this.currentData) return;
        try {
            localStorage.setItem('dv-data', JSON.stringify(this.currentData));
            localStorage.setItem('dv-data-timestamp', Date.now().toString());
        } catch (e) {
    ...
    /**
     * Save current data to localStorage with debounce (optimization)
     * Prevents excessive writes when data changes frequently
     */
    ...
    /**
     * Force save immediately (cancels pending debounced save)
     ...
     * @returns {object|null} The saved data or null if none exists or expired
     */
    loadFromStorage() {
        try {
            const saved = localStorage.getItem('dv-data');
            if (saved) {
                const timestamp = localStorage.getItem('dv-data-timestamp');
                const savedTime = timestamp ? parseInt(timestamp) : null;

                // Check for data expiry (optimization: auto-cleanup old data)
                if (savedTime && Date.now() - savedTime > this._dataExpiry) {
                    console.log('Saved data expired, clearing storage');
                    this.clearStorage();
                    return null;
                }
     ...
     */
    hasSavedData() {
        return localStorage.getItem('dv-data') !== null;
    }
     ...
     */
    clearStorage() {
        localStorage.removeItem('dv-data');
        localStorage.removeItem('dv-data-timestamp');
    }
